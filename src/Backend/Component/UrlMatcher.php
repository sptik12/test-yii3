<?php
/**
 * This class is a clone of yiisoft/router-fastroute/UrlMatcher, which solves the getHost() problem:
 * https://github.com/yiisoft/router-fastroute/issues/151
 * TODO: Remove this class after fix getHost() problem in yiisoft/router-fastroute
 */
...
final class UrlMatcher implements UrlMatcherInterface
{
    ...
    public function match(ServerRequestInterface $request): MatchingResult
    {
        ...
        $isDebugPanel = str_contains($path, "/debug") || str_contains($path, "/inspect");
        $result = $this
            ->getDispatcher($dispatchData)
            ->dispatch($method, !$isDebugPanel ? "{$host}{$port}{$path}" : "{$host}{$path}");
